...
describe('SpecialityService', () => {
  ...
  describe('findOneSpeciality', () => {
    ...
    it('should handle error if speciality not found', async () => {
      ...
      try {
        await service.findOneSpeciality(id);
      } catch (error) {
  ...
  describe ('update',()=>{
    it('should call update speciality with correct params',async() =>{
      ...
      const existingSpe: Speciality = { ...speciality,createId:jest.fn()};
      const updatedSpe:Speciality = {...speciality,createId:jest.fn()};
      const result: IResponse = {
      ...
      expect(repository.update).toHaveBeenCalledWith(id,speciality);
    })
    it('should handle error during update',async ()=> {
      const id= '89820e';
      jest.spyOn(repository,'update').mockRejectedValue(new Error("No se pudo actualizar la especialidad"))
      try {
        await service.updateSpeciality(id,speciality);
      } catch (error) {
    ...
    it('should handle non-existing speciality', async () => {
      ...
      try {
        await service.deleteSpeciality(id);
      } catch (error) {
